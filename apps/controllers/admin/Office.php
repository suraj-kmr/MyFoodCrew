...
class Office extends MY_Controller{
    ...
    function index(){
        ...
        $this -> data['main'] = admin_view('office/index');
        $status = isset($_GET['status']) ? $_GET['status'] : 'all';
        $rule = array();
        if($status == 'active'){
            $rule['status'] = 1;
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset, $rule, 7);
        }elseif($status == 'inactive'){
            $rule['status'] = 0;
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset, $rule, 7);
        }else{
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset,'', 7);
        }
        ...
        if($this -> input -> get('btnsearch')){
            ...
            if($q <> ''){
                ...
                $data = $this -> Product_model -> getAllSearchedWhere($offset, $show_per_page, $likes, array('product_type'=>OFFICEPRODUCTS));
                $this -> data['q'] = $q;
            }
        }

        $this -> data['products'] = $data['results'];
        $config['base_url'] 	 = admin_url('office/index');
        $config['num_links'] 	 = 2;
    ...
    function add($id = false){
        ...
        $this -> data['active_tabs'] ='office';
        $this -> data['main'] = admin_view('office/add');
        $this -> data['dashboard_title'] = ($id == false) ? "Add Products" : "Edit Products";
        $this -> data['brand'] = $this -> Master_model -> listAll('ai_obrand');
        $this -> data['material'] = $this -> Master_model -> listAll('ai_omaterial');

        $this -> data['theme'] = $this -> Master_model -> listAll('ai_otheme');
        $this -> data['categories']		= $this -> Category_model -> get_categories_tierd(0, 7);
        ...
        if($this -> form_validation -> run()){
            ...
            $office['id'] = false;
            if($id){
                $office1 = $this -> Master_model -> getBookId($id, 'ai_office');
                $office['id'] = $office1 -> id;
            }
            $this -> Master_model -> save($office, 'ai_office');
            ...
            redirect(admin_url('office/add/'. $id));
        }
    ...
    function brand(){
        ...
        $show_per_page = 50;
        $offset = ($page - 1 ) * $show_per_page;
        $this -> data['main'] = admin_view('office/brand');
        $data = $this -> Master_model -> getAll($offset, $show_per_page,'ai_obrand');
        $this -> data['q'] = '';
        if($this -> input -> get('btnsearch')){
            $q = $this -> input -> get('q');
            if($q <> ''){
                ...
                $data = $this -> Master_model -> getAllSearched($offset, $show_per_page, $likes, 'ai_obrand');
                $this -> data['q'] = $q;
            }
        }

        $config['base_url'] 	 = admin_url('office/brand');
        $config['num_links'] 	 = 2;
        $config['uri_segment']	 = 4;
        $config['total_rows']	 = $data['total'];
        $config['per_page'] 	 = $show_per_page;
    ...
    function brand_add($id = false){
        $this -> data['brand'] = $this -> Master_model -> getNew('ai_obrand');
        if($id){
            $this -> data['brand'] = $this -> Master_model -> getRow($id, 'ai_obrand');
        }
        $this -> data['main'] = admin_view('office/add_brand');
        $this -> form_validation -> set_rules('frm[title]', 'Brand', 'required');
        if($this -> form_validation -> run()) {
            $data = $this->input->post('frm');
            $data['id'] = $id;
            $this->Master_model->save($data, 'ai_obrand');
            $this->session->set_flashdata('success', "Successfully Saved");
            redirect(admin_url('office/brand_add/'. $id));
        }
        else {
            $this->load->view(admin_view('default'), $this->data);
        }
    ...
    function material(){
        ...
        $show_per_page = 50;
        $offset = ($page - 1 ) * $show_per_page;
        $this -> data['main'] = admin_view('office/material');
        $data = $this -> Master_model -> getAll($offset, $show_per_page,'ai_omaterial');
        $this -> data['q'] = '';
        if($this -> input -> get('btnsearch')){
            $q = $this -> input -> get('q');
            if($q <> ''){
                ...
                $data = $this -> Master_model -> getAllSearched($offset, $show_per_page, $likes,'ai_omaterial');
                $this -> data['q'] = $q;
            }
        }

        $config['base_url'] 	 = admin_url('office/material');
        $config['num_links'] 	 = 2;
        $config['uri_segment']	 = 4;
        $config['total_rows']	 = $data['total'];
        $config['per_page'] 	 = $show_per_page;
    ...
    function material_add($id = false){
        $this -> data['material'] = $this -> Master_model -> getNew('ai_omaterial');
        if($id){
            $this -> data['material'] = $this -> Master_model -> getRow($id,'ai_omaterial');
        }
        $this -> data['main'] = admin_view('office/add_material');
        $this -> form_validation -> set_rules('frm[title]', 'Material', 'required');
        if($this -> form_validation -> run()) {
            $data = $this->input->post('frm');
            $data['id'] = $id;
            $this->Master_model->save($data, 'ai_omaterial');
            $this->session->set_flashdata('success', "Successfully Saved");
            redirect(admin_url('office/material_add/'. $id));
        }
        else {
            $this->load->view(admin_view('default'), $this->data);
        }
    ...
    function theme(){
        ...
        $show_per_page = 50;
        $offset = ($page - 1 ) * $show_per_page;
        $this -> data['main'] = admin_view('office/theme');
        $data = $this -> Master_model -> getAll($offset, $show_per_page,'ai_otheme');
        $this -> data['q'] = '';
        if($this -> input -> get('btnsearch')){
            $q = $this -> input -> get('q');
            if($q <> ''){
                ...
                $data = $this -> Master_model -> getAllSearched($offset, $show_per_page, $likes,'ai_otheme');
                $this -> data['q'] = $q;
            }
        }

        $config['base_url'] 	 = admin_url('office/theme');
        $config['num_links'] 	 = 2;
        $config['uri_segment']	 = 4;
        $config['total_rows']	 = $data['total'];
        $config['per_page'] 	 = $show_per_page;
    ...
    function theme_add($id = false){
        $this -> data['theme'] = $this -> Master_model -> getNew('ai_isbm');
        if($id){
            $this -> data['theme'] = $this -> Master_model -> getRow($id, 'ai_otheme');
        }
        $this -> data['main'] = admin_view('office/add_theme');
        $this -> form_validation -> set_rules('frm[title]', 'Theme', 'required');
        if($this -> form_validation -> run()) {
            $data = $this->input->post('frm');
            $data['id'] = $id;
            $this->Master_model->save($data, 'ai_otheme');
            $this->session->set_flashdata('success', "Successfully Saved");
            redirect(admin_url('office/theme_add/'. $id));
        }
        else {
            $this->load->view(admin_view('default'), $this->data);
        }
    ...
    function activate($id = false){
        $redirect = isset($_GET['redirect_to']) ? $_GET['redirect_to'] : admin_url('office');
        if($id){
            $c['id'] = $id;
            $c['status'] = 1;
    ...
    function bdelete($id){
        $this -> Master_model -> delete($id,'ai_obrand');
        $this -> session -> set_flashdata('success', 'Data Deleted Successfully');
        redirect(admin_url('office/brand'));
    }

    function mdelete($id){
        $this -> Master_model -> delete($id,'ai_omaterial');
        $this -> session -> set_flashdata('success', 'Data Deleted Successfully');
        redirect(admin_url('office/material'));
    }

    function tdelete($id){
        $this -> Master_model -> delete($id,'ai_otheme');
        $this -> session -> set_flashdata('success', 'Data Deleted Successfully');
        redirect(admin_url('office/theme'));
    }
