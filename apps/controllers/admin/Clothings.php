<?php
class Clothings extends MY_Controller{
    ...
    function index(){
        ...
        $this -> data['main'] = admin_view('clothings/index');
        $status = isset($_GET['status']) ? $_GET['status'] : 'all';
        $rule = array();
        if($status == 'active'){
            $rule['status'] = 1;
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset, $rule, CLOTHINGS);
        }elseif($status == 'inactive'){
            $rule['status'] = 0;
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset, $rule, CLOTHINGS);
        }else{
            $data	= $this -> Product_model -> getWherePtype($show_per_page, $offset,'', CLOTHINGS);
        }
        ...
        if($this -> input -> get('btnsearch')){
            ...
            if($q <> ''){
                ...
                $data = $this -> Product_model -> getAllSearchedWhere($offset, $show_per_page, $likes, array('product_type'=>CLOTHINGS));
                $this -> data['q'] = $q;
            }
        }
        $this -> data['products'] = $data['results'];
        $config['base_url'] 	 = admin_url('clothings/index');
        $config['num_links'] 	 = 2;
	...
	function combo(){
        ...
        $page = isset($_GET['page']) ? $_GET['page'] : 1;
        $offset = ($page - 1 ) * $show_per_page;
        $this -> data['main'] = admin_view('clothings/combo');
        $status = isset($_GET['status']) ? $_GET['status'] : 'all';
        $rule = array();
        if($status == 'active'){
            $rule['status'] = 1;
        ...
        $this -> data['products'] = $data['results'];
        $config['base_url'] 	 = admin_url('clothings/combo');
        $config['num_links'] 	 = 2;
        $config['uri_segment']	 = 4;
        $config['total_rows']	 = $data['total'];
        $config['per_page'] 	 = $show_per_page;
    ...
    function add($id = false){
        ...
        //$this -> data['active_tabs'] ='clothings';
        $this -> data['main'] = admin_view('clothings/add');
        $this -> data['dashboard_title'] = ($id == false) ? "Add Products" : "Edit Products";
        $this -> data['categories']		= $this -> Category_model -> get_categories_tierd(0, CLOTHINGS);
        ...
        if($this -> form_validation -> run()){
            ...
            redirect(admin_url('clothings/add/'. $id));
        }

        $this -> load -> view(admin_view('default'), $this -> data);
    }
	
	function combo_add($id = false){
        ...
        $this->load->library('upload', $config);
        //$this -> data['active_tabs'] ='clothings';
        $this -> data['main'] = admin_view('clothings/combo-add');
        $this -> data['dashboard_title'] = ($id == false) ? "Add Products" : "Edit Products";
        $this -> data['categories']		= $this -> Category_model -> get_categories_tierd(0, CLOTHINGS);
        $this -> data['gift'] = $this -> Master_model -> listAllWhere('categories', array('parent_id'=>102));
        $this -> data['images'] = $this -> Media_model -> allimages();
        ...
        if($this -> form_validation -> run()){
            ...
            if($p['slug'] == 'combo-tshirts')
            {
                $s['prod_id'] = $id;
                $s['lbl1'] = $_POST['lbl1'];
                $s['lbl2'] = $_POST['lbl2'];
                $s['price1'] = $_POST['price1'];
                $s['price2'] = $_POST['price2'];
                $combo = $this->db->get_where('combo_prod', array('prod_id' =>$id));
                if($combo->num_rows() > 0)
                {
                    $this->db->update('combo_prod', $s, array('prod_id' => $id));
                }else{
                    $this->db->insert('combo_prod', $s);
                }
            }

            $this -> session -> set_flashdata("success", "Product saved successfully");
            redirect(admin_url('clothings/combo_add/'. $id));
        }
    ...
    function activate($id = false){
        $redirect = isset($_GET['redirect_to']) ? $_GET['redirect_to'] : admin_url('clothings');
        if($id){
            $c['id'] = $id;
            $c['status'] = 1;
