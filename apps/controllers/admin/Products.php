...
class Products extends MY_Controller{
    ...
    function add($id = false){
       ...
       // $this -> data['brands'] = $this -> Mobile_model -> allBrands();
        ...
        if($this -> form_validation -> run()){
            ...
            // $w=$this->db->get_where('products',array('id'=>$id))->row();
            // if($w->casback_type==1){
            //    $a=$w->price-$w->discount_amount;
            // }else{
            //        $p=$w->price * $w->discount_amount / 100;
            //        $amt=$w->price-$p;
            // }
 ...
 function create_package($id=false){
    ...
    {
        //print_r($_POST);
        $arr=array('id'=>$_POST['id'],'m_featured'=>$_POST['status']);
    ...
    // function cashback(){
    //     $uid=$this->db->order_by('id','DESC')->get_where('orders'array('user_id'=>user_id()))->row();
    //     $order_id=$uid->id;
    //     $pid=$this->db->get_where('order_items',array('order_id'=>$order_id))->row();
    //     $product_id=$pid->product_id;
    //      $pid=$this->db->get_where('products',array('id'=>$product_id))->row();
