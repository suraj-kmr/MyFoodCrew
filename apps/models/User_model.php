...
class User_model extends Master_model{
	...
	function loginCheck($username, $pass){
		$this -> db -> where("(users.username='$username' OR users.email_id = '$username')");
    ...
    function chkMob($mobile){
    	$mob = $this->db->get_where('users',array('phone_no'=>$mobile))->row();
    	if($mob){
    		return true;
    	}
    	else
    ...
    function getUserdata($mobile){
    	return $this->db->get_where($this->table,array('phone_no'=>$mobile))->row();
    }

    function save_otp($otp,$mobile){
    	$arr = array(
    		'otp'=>$otp,
    	);
