...
class Category_model extends Master_model
{
    ...
    function getSubCat($parent){
        return $this -> db -> get_where('categories', array('parent_id'=>$parent))->result();
    }
    ...
    function categories($parent = 0, $product_type = false)
    {
       ...
       // return $this->db->get_where($this->table, array('id' => $parent))->result();

    }

    function get_categories_tierd1($parent = 0, $product_type = false)
    {

        $categories = array();

        $result = $this->categories($parent, 2);
    ...
    function giftCat($cat_id)
    {
        $this->db->order_by('sequence', 'ASC');
        return $this->db->get_where('categories', array('product_type' => ''))->result();
    }
    ...
    function getParentId($id){
        $c = $this -> db -> get_where($this -> table, array('id' => $id))->row();
        return $c -> parent_id;
    }
    ...
    function getIdByEvent($event){
        ...
        return $this -> db -> get('ai_event')->row()->id;
    }
