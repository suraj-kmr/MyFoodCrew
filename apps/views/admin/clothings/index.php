<div class="row">
    ...
    <div class="col-sm-3">
        <a class="btn btn-success" href="<?= admin_url('clothings/export_all'); ?>">Export All Products</a>
    </div>
...
<div class="box box-header box-status">
    ...
    <label class="checkbox-inline">
        <form name="show" method="get" action="<?= admin_url('clothings'); ?>">
    ...
    <a href="<?= admin_url('clothings/import_files'); ?>" style="margin-left:5px;" class="btn btn-sm btn-warning pull-right"> <i class="fa fa-upload"></i> Import File</a>
    <a href="<?= admin_url('clothings/add'); ?>" class="btn btn-sm btn-primary pull-right"><i class="fa fa-plus-circle"></i> Add New Product</a>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.box-status input').on('click', function(){
            var v = $(this).val();
            if(v == 1){
                window.location.href="<?= admin_url('clothings/index?status=active'); ?>"
            }else if(v == 0){
                window.location.href="<?= admin_url('clothings/index?status=inactive'); ?>"
            }else{
                window.location.href="<?= admin_url('clothings/index'); ?>"
            }
        });
...
<div class="row form-search">
    <div class="col-sm-6">
        <form method="get" action="<?= admin_url ('clothings'); ?>">
...
<form id="frmsave" method="post" action="<?= admin_url('clothings/bulksave'); ?>">
                    ...
                    foreach($products as $p){
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if($p -> status == 1){
                                    ?>
                                    <a href="<?= admin_url('office/deactivate/' . $p -> id, true); ?>" class="label label-success">Active</a>
                                <?php
                                }else{
                                    ?>
                                    <a href="<?= admin_url('office/activate/' . $p -> id, true); ?>" class="label label-danger">Deactive</a>
                                <?php
                                }
                                ?>
                            </td>
                            ...
                            <td>
                                <div class="btn-group pull-right">
                                    ...
                                    <a href="<?= admin_url('clothings/add/'. $p -> id); ?>" title="Edit" class="btn btn-xs btn-info"><i class="fa fa-pencil"></i> </a>
                                    <a href="<?= admin_url('clothings/delete/'. $p -> id); ?>" title="Delete" class="btn btn-xs btn-danger delete"><i class="fa fa-trash"></i> </a>
                                </div>
                            </td>
                        </tr>
                    <?php
...
<script type="text/javascript">
    ...
    function submit(type){
        ...
        if (r == true) {
            var url = '<?php echo admin_url('office'). '/'; ?>' + type;
    ...
    function submit1(type){
        ...
        var url = '<?php echo admin_url('clothings/') ?>' + type;
        $('#frmsave').attr('action', url).submit();
