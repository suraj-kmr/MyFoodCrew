...
<div class="row">
	<div class="col-md-12">
		...
		<a href="#" style="margin-left: 5px" class="btn btn-sm btn-warning pull-right btn-save-all"><i class="fa fa-save"></i> Save All</a>
			<a href="<?php echo site_url('admin/mobiles/add'); ?>" class="btn btn-sm btn-primary tooltips" title="Add New">
...
<div class="row">
    <div class="col-sm-5">
        <form method="get" action="<?= admin_url ('mobiles'); ?>">
...
<input type="hidden" name="frmall" value="Save All" />
	...
	<div class="box">
		...
		<div class="box-header">
			...
			List Ads ( Maximum sequence used: <b><?php echo $max_sequence->sequence; ?></b> )
		</div>
				...
				<?php if (count($data) > 0) { ?>
					<?php foreach($data as $row) { ?>
						...
						<tr>
							...
							<td>
								<?php echo $this -> Mobile_model -> brand($row -> brand_id); ?>
							</td>
							<td><input type="number" name="sequence[<?= $row->id; ?>]"  value="<?= $row -> sequence; ?>" class="form-control input-sm" style="width: 70px;" /></td>
							<td class="center">
								<?php if ($row->status == 1){ ?>
							...
							<td class="center">
								<div class="btn-group">
									<a title="Edit" class="btn btn-xs btn-default" href="<?php echo admin_url('mobiles/add') . '/' . $row->id; ?>"><i class="fa fa-pencil"></i> </a>
									<a title="Delete" class="btn btn-xs btn-danger delete" href="<?php echo admin_url('mobiles/delete') . '/' . $row->id; ?>"><i class="fa fa-trash"></i></a>
								</div>
							</td>
						</tr>
